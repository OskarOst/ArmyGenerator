...
namespace ArmyGenerator
{
    ...
    class BaseHero : BaseHumanoid
    {
        ...
        private int hero;
        private int leader; 
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, int leader, int hero, int army) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name,army)
        {
            AddEquipment(0,0, leader, hero, null);
        }
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, string rules, int leader, int hero, int army) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name, army)
        {
            AddEquipment(0,0, leader, hero, null);
        }
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, int mana, int spells, int leader, int hero, int army) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name, army)
        {
            AddEquipment(mana, spells, leader, hero, null);
            
        }
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, string rules, int mana, int spells, int leader, int hero, int army) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name, army)
        {
            AddEquipment(mana, spells, leader, hero, null);
        }
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, string rules, int mana, int spells, int though, int mighty, int size, int leader, int hero, int army) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name, though, mighty, size, army)
        {
            AddEquipment(mana, spells, leader, hero, null);
        }
        public BaseHero(int cost, int fight, int shoot, int defence, int courage, int discipline, int minSpeed, int maxSpeed, string name, string rules, int mana, int spells, int though, int mighty, int size, int leader, int hero, int army, List<SpellsClass> spellList) : base(cost, fight, shoot, defence, courage, discipline, minSpeed, maxSpeed, name, though, mighty, size, army)
        {
            AddEquipment(mana, spells, leader, hero,spellList);
        }
        public BaseHero(BaseHero input) : base(input)
        {
            AddEquipment(input.mana, input.spells, input.leader, input.hero, input.spellList);
        }
        ...
        public override void EquipUnit(Equipment input)
        {
            if (input is HeroTraits)
            {
                ...
                else
                {
                    ...
                    if (input.ModifyStats(0, 11) > 0)
                    {
                        while (spellList.Count < spells + input.ModifyStats(0, 11))
                        {
                            spellList.Add(new SpellsClass());
                        }
                    }
                }
            }
        ...
        public override void RemoveEquipment(Equipment output)
        {
            if (output is HeroTraits)
            {
                ...
                else if (output.ModifyStats(0, 11) > 0)
                {
                    while (spellList.Count > spells)
                    {
                        spellList.RemoveAt(0);
        ...
        public override string GetRules()
        {
            ...
            if (mana > 0)
            {
                tempstring += "Mana: (" + mana.ToString() + ")";
            }
            if (spells > 0)
            {
                tempstring += ", Spells: (" + spells.ToString() + ")";
            }
            return base.GetRules() + ", Hero (" + hero.ToString() + "), Leader: (" + leader.ToString() + "), " + tempstring;
        }
        ...
        private void AddEquipment(int mana, int spells, int leader, int hero, List<SpellsClass> spellList)
        {
